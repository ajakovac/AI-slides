...
- **reinforcement learning (RL)**
    ...
    - application areas:
        ...
        - language models
            - initial (fully or semi-) supervised training
            - fine-tuning with reinforcement learning using human preferences (RLHF)
    - **core components**
        - **agent**  
        the decision-making entity (e.g. player, driver, robot, language model)
        - **environment**  
        ...
        - **action**  
        an element of the action space available to the agent;  
        actions influence the future state of the environment
        - **reward**  
        a scalar feedback signal measuring immediate success or failure  
        (e.g. win: +1, draw: 0, loss: −1)

    - **policy** (decision mechanism)
        - a mapping from states (or observations) to actions
        - learned to **maximize the expected cumulative (long-term) reward**
    ...
    - **reward hypothesis**  
        ...
        - the objective is typically **non-local in time**:
          future rewards are taken into account via discounting
        - challenges:
            - **exploration issues**: the agent may exploit loopholes or glitches
            - **degenerate optima**: e.g. preferring safe draws over risky wins
            - **sparse or rare rewards**: important events may occur infrequently,
...
- **policy**
    ...
    - **exploration vs. exploitation**
        ...
        - **exploration**: try alternative actions to acquire new information
...
- **evaluation measures** (value functions)
...
- **Dynamic Programming**
    ...
    - algorithm:
        - for a given policy obtain the the state-value function (Bellmann equation)
        - for an optimal state-value function get the optimal policy (Optimal Policy from the Bellman Equation)
        - alternative algorthms: Q-learning, SARSA
    - remark: game theory gives a similar approach
        - Bellmann equation $\to$ Nash equilibrium (in ideal cases)
    - critics:
        - not full information game
        - not fully known reward function
        - the state and action space is too large
        - solution: Monte Carlo control, TD learning
...
- **Bellman Equation**
    ...
    - simplified form when we choose only the best strategy (**Bellman optimality equation**)
    $$ V^*(s)=\max_{a}\sum_{s'} P(s' \mid s, a)\bigl[r(s,a,s') + \gamma V^*(s')\bigr]$$
    - for two-player game: $$V(s) = \max_{a}\min_{a'}[r(s,a,a') + \gamm V(s)]
...
- **SARSA** (State–Action–Reward–State–Action)
    - a **model-free, on-policy** reinforcement learning algorithm
    ...
    - intuition:
        - *“Learn the value of what you actually do.”*

- **Two-Player Games (Zero-Sum)**
...
- **board games and computer games (AI)**
    - evaluation of a strategy is typically available only at the end of the game (e.g. win / loss)
    - successful AI agents:
        - chess engines: Leela, Stockfish
          Elo rating ≈ 3500 (best human ≈ 2800)
        - Go: AlphaGo
        - Dota 2: OpenAI Five → world-champion level
        - StarCraft II: AlphaStar → world-champion level
    - in closed-information games, AI systems can outperform human players

- **robotics** (in reinforcement learning)
    - success of a sequence of actions is often observed only at the end  
...
- **Leela** Chess Zero
    ...
    - Project page: https://lczero.org
    - Code: https://github.com/LeelaChessZero/lc0
...
- **Stockfish**
    ...
    - Remark:
    Stockfish is primarily a classical search + evaluation engine, but modern versions integrate neural-network-based evaluation (NNUE).
...
- **OpenAI Five** (Dota 2)
    ...
    - Blog / overview: https://openai.com/research/openai-five
...
- **AlphaStar**
    ...
    - Blog: https://deepmind.google/discover/blog/alphastar-mastering-the-real-time-strategy-game-starcraft-ii/


- **Monte Carlo control** (in RL)
    - **motivation**
        - Dynamic Programming requires:
            - full knowledge of transition probabilities $P$
            - complete sweeps over the state space
...
- **Monte Carlo Tree Search**, **MCTS**
    ...
    - **algorithm**
        ...
        1. Selection
            ...
            - continue until a node is reached that:
                - is not fully expanded, or
                - is terminal

            - UCT upper confidence bound (classical selection rule)
        ...
        1. Expansion: if the selected node is non-terminal and not fully expanded:
        ...
        1. Backpropagation
            ...
            - update statistics for each visited node (or edge):
                - visit counts
                - cumulative rewards
            - in two-player zero-sum games:
                - alternate the sign of $z$ when moving up the tree
        1. recursion: repeat this loop from many simulations
    - Output of MCTS
        - after many simulations, select at the root:
            - the action with the **highest visit count**, or
            - the action with the **highest mean value**
    - Key properties
        - combines:
            - search (MCTS)
...
- **PUCT** Policy-guided MCTS (AlphaZero-style)
    - **motivation**
