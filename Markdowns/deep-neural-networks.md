...
- **CNNs**
    ...
    - **see**: __DNNs__, __layer types__

- **layer types**
    > we can use different layer types performing different tasks
...
- **backpropagation**
    > derivative in a heirarchical structure can be computed recursively
    - **algorithm**
...
- **vanishing gradient**
    ...
    - **formal cause**
        - __backpropagation__ gradients $||U||<1$ (spectral norm is smaller than one)
    ...
    - **technical causes**
        ...
        - poor __weight initialization__
    - **consequences**
    ...
    - **mitigation techniques**
        ...
        - use better __activation functions__ (ReLU and similar) to avoid saturation and to control gradient magnitude
        - use __normalization techniques__
        - use __residual connections__
...
- **activation functions**
    ...
    - **link**: [pytorch activation functions](https://docs.pytorch.org/docs/main/nn.html#non-linear-activations-weighted-sum-nonlinearity)

- **weight initialization**
    > how to choose the initial values of the weight 
    ...
    - **effect**: balances forward and backward signal propagation.


- **normalization techniques**
    > reweight the layers in order to achieve a more stable training, to keep gradients well-conditioned
    - **types**: batch normalization, layer normalization, and related methods.
    - **effects**
...
- **gradient clipping**
    > constrain the valus of the gradient
    - **note**: used especially in recurrent neural networks
    - **strategy**: allow maximal value for gradient $\|\nabla\| \leftarrow \min(\|\nabla\|, \tau)$
    - **benefits**
        - prevents exploding gradients
        - stabilizes training

- **symmetries of the data**
    > the data may have special symmetries we know before training; we can improve training a lot if we take into account these symmetries in the architecture
    - **observation**: __DNNs__ can represent symmetries,
    ...
    - **architectural examples**
      - translation invariance $\to$ __CNNs__
      - scaling invariance $\to$ pooling (__layer types__)
...
- **pretrained models**
    > if we solved classification for a similar problem we may start training from that architecture
    - **motivation**: training __DNNs__ from scratch is computationally expensive
      and requires large labeled datasets
    - **idea**: reuse a model (or part of a model) trained on a large, generic dataset
      and adapt it to a new, related task
    - **assumption**: lower-level representations (features) are transferable across tasks
    - **typical workflow**
...
- **catastrophic forgetting**
    >  during sequential training on multiple tasks, a neural network rapidly loses performance on previously learned tasks when trained on new ones
    - **cause**
    ...
    - **intuition**: gradient updates for the new task interfere destructively
      with representations learned for previous tasks
    - **example**: a classifier trained on task A, then on task B,
        forgetting task A almost completely
    - **consequence**
      - inability to accumulate knowledge over time
      - poor long-term learning
    - **solution**
      - regularization-based: penalize changes to important parameters (e.g. elastic weight consolidation)
      - rehearsal-based: replay old data or synthetic samples
      - architectural: task-specific subnetworks, expandable models
      - representation-based: learn task-invariant features

- **error-prone behavior**
    > deep neural networks can be highly sensitive to small, carefully chosen perturbations of the input
    - **definition (adversarial example)**: an input modified by an imperceptible perturbation
      that causes a confident but incorrect prediction
    - **cause**
      - high-dimensional input spaces
      - locally linear behavior of __DNNs__
      - decision boundaries close to the data manifold
    - **intuition**: small input changes can accumulate along many dimensions
      and push the input across a decision boundary
    - **examples**
      - image perturbations invisible to humans but misclassified by __DNNs__
    ...
    - **solution**: __solution for error-prone behavior__

- **solution for error-prone behavior**
    > we can mitigate the error sensibility of the model
...
- **adversarial attacks**
    > we can plan an attack against a classifier knowing its architecture
    - **see**: __error-prone behavior__