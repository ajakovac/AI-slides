...
window.addEventListener("popstate", (event) => {
  ...
  if (key) {
    ...
    // If no state, maybe load default
    const first =
      document.querySelector("nav [data-key]") ||
      document.querySelector("[data-key]");
    if (first) {
      renderKeyIntoBlock(first.dataset.key, "block");
    }
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (keyFromUrl) {
    renderKeyIntoBlock(keyFromUrl, "block");
    history.replaceState({ key: keyFromUrl }, "", window.location.href);
    return;
  }
  ...
  if (first) {
    ...
    renderKeyIntoBlock(key, "block");
    history.replaceState({ key }, "", `?key=${encodeURIComponent(key)}`);
  }
});
...
//  if (!el) return;
//  e.preventDefault();
//  renderKeyIntoBlock(el.dataset.key, "block");
//});
//
//// --------- auto-render first key on page load ---------
//document.addEventListener("DOMContentLoaded", () => {
//  // Prefer a key inside <nav>, fallback to any element with data-key
//  const first =
//    document.querySelector("nav [data-key]") ||
//    document.querySelector("[data-key]");
//
